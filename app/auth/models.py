...
class AdminSession(Base):
    """Sesiones admin con contraseña temporal"""
    __tablename__ = "admin_sessions"
    
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)  
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)  
    session_password_hash = Column(String(128), nullable=False)
    session_password_salt = Column(String(64), nullable=False)
    session_password_expires_at = Column(DateTime(timezone=True), nullable=False)  
    ip_address = Column(String(45), nullable=False)
